...
type ScenicShow = {
  title: string
  description: string
  date: string  // raw text like "April 21"
  ticketUrl?: string
}

async function scrapeScenic(): Promise<ScenicShow[]> {
  try {
    ...
    // Extract h4 blocks — each is one event
    ...
    for (const h4Match of h4Matches) {
      ...
      // Find the content after this h4 until next h4
      ...
      // Find a date in the block (e.g. "April 21" or "May 3")
...
function parseSceniDate(rawDate: string, year: number): string | null {
...
export async function GET(req: NextRequest) {
  ...
  const [tmShows, scenicShows] = await Promise.all([
    scrapeTicketmaster(),
    scrapeScenic(),
  ])

  // Build a Scenic lookup by date → description
  const year = new Date().getFullYear()
  const scenicByDate: Record<string, ScenicShow> = {}
  for (const s of scenicShows) {
    const date = parseSceniDate(s.date, year)
    if (date) scenicByDate[date] = s
    // also try next year for shows in Jan-Mar when current month is late in year
    if (!date) {
      const dateNext = parseSceniDate(s.date, year + 1)
      if (dateNext) scenicByDate[dateNext] = s
    }
  }

  // Merge: attach Scenic descriptions to Ticketmaster shows
  ...
  // Also add any Scenic shows not in Ticketmaster (with dates only)
  ...
  return NextResponse.json({ shows: unique, count: unique.length, scenicCount: scenicShows.length })
}
